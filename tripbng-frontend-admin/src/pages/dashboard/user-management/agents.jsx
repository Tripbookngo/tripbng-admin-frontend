...
const AgentsPage = () => {
  ...
  const fetchAgents = async () => {
    ...
    try {
      ...
      if (response.ok && result.data && result.data.data) {
        ...
        setApprovedAgents(result.data.data.AproveAgents || []);
      } else {
        toast.error("No agents data found.");
      }
    } catch (error) {
  ...
  return (
    <>
      <HeaderNav title="Agents Management" onSearch={handleSearch} />
      <div className="p-4">
        {loading ? (
          ...
          <>
            ...
            {renderAgentTable(approvedAgents, "Approved Agents")}
          </>
        )}
      </div>
    </>
